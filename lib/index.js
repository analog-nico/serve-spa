...
module.exports = function initServeSpa(appOrRouter, rootPath, options) {
    ...
    } else if (!_.isString(rootPath) && !_.isArray(rootPath)) {
        throw new TypeError('The root path must be of type string.');
    } else if (_.isString(rootPath) && !pathIsAbsolute(rootPath)) {
        throw new TypeError('The root path must be absolute.');
    } else if (_.isArray(rootPath)) {
        if (rootPath.length === 0) {
            throw new TypeError('The root path array must contain at least one path.');
        }
        _.forEach(rootPath, function (onePath, i) {
            if (!_.isString(onePath)) {
                throw new TypeError('The root path at index ' + i + ' must be of type string.');
            } else if (!pathIsAbsolute(onePath)) {
                throw new TypeError('The root path at index ' + i + ' must be absolute.');
            }
        });
    }
    ...
    _.forEach(rootPath, function (onePath) {

        var templates = glob.sync('**/index.htmlt', { cwd: onePath });
        _.forEach(templates, function (template) {
            ...
            try {
                cache[pathToUri(dir)] = {
                    template: _.template(fs.readFileSync(path.join(onePath, template)).toString(), options.templateSettings)
                };
            } catch (e) {
                throw new Error('Failed to parse the template "' + path.join(onePath, template) + '" with error: ' + e.message);
            }

            var composefile = path.join(onePath, path.join(dir, 'compose.js'));
    ...
    _.forEach(rootPath, function (onePath) {
        ...
        var files = glob.sync('**/*', { cwd: onePath });
    ...
    _.forEach(rootPath.reverse(), function (onePath) {
        appOrRouter.use(serveStatic(onePath, options.staticSettings));
    });
