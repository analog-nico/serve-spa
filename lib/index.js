...
module.exports = function initServeSpa(app, root, options) {
    ...
    _.forEach(templates, function (template) {
        ...
        try {
            cache[pathToUri(dir)] = {
                template: _.template(fs.readFileSync(path.join(root, template)).toString(), options.templateSettings)
            };
        } catch (e) {
            throw new Error('Failed to parse the template "' + path.join(root, template) + '" with error: ' + e.message);
        }

        var composefile = path.join(root, path.join(dir, 'compose.js'));
    ...
    function serveSpa(req, res, next) {
        ...
        function render() {
            ...
            if ((arguments.length === 1 && !_.isUndefined(arguments[0]) && !_.isNull(arguments[0])) || arguments.length > 1) {
            ...
            try {
                ...
                var html = cache[cacheEntry].template(data);
        ...
        // Don't serve index.htmlt and compose.js directly
        if (url.pathname.match(/\/(index.htmlt|compose.js)$/i) !== null) {
        ...
        if (!_.isUndefined(cache[cacheEntry].compose)) {
            cache[cacheEntry].compose(req, res, render);
        } else {
            render();
        }
